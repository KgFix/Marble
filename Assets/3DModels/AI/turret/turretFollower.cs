...
public class turretFollower : MonoBehaviour
{
    ...
    public ParticleSystem muzzleFlash; // Assign in Inspector or find on firePoint
    ...
    void Update()
    {
        if (player != null && CanSeePlayer())
        {
            ...
            // Rotate the turret to look at the predicted position (only on the Y axis)
            Vector3 lookPos = targetPos - transform.position;
            lookPos.y = 0; // Keep turret level, only rotate horizontally
        ...
        {
            // Optionally, you can add idle behavior here if needed
        }
    }
